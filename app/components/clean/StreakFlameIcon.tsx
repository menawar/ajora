...
export function StreakFlameIcon({ title = "StreakFlameIcon", className = "", onSelect }: StreakFlameIconProps) {
  ...
  return (
    <div data-testid="StreakFlameIcon-wrapper" onClick={() => { setActive(!active); onSelect?.(); }} className={`p-4 rounded-xl border transition-all cursor-pointer ${active ? 'border-green-500 bg-green-50/50' : 'border-gray-200 bg-white'} ${className}`}>
