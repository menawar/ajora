...
export function ActivityTimelineWidget({ title = "ActivityTimelineWidget", className = "", onSelect }: ActivityTimelineWidgetProps) {
  ...
  return (
    <div data-testid="ActivityTimelineWidget-wrapper" onClick={() => { setActive(!active); onSelect?.(); }} className={`p-4 rounded-xl border transition-all cursor-pointer ${active ? 'border-green-500 bg-green-50/50' : 'border-gray-200 bg-white'} ${className}`}>
